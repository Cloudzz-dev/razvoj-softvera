...
export function ModernFooter({ compact = false }: ModernFooterProps) {
    const currentYear = new Date().getFullYear();

    return (
        <footer className={`relative border-t border-white/10 overflow-hidden ${compact ? 'py-4 bg-black/20 backdrop-blur-md' : 'pt-16 pb-8'}`}>
            <div className="container px-4 md:px-6 mx-auto relative z-10">
                ...
                <div className={`${compact ? '' : 'pt-8 border-t border-white/10'} flex flex-col md:flex-row justify-between items-center gap-4`}>
                    <div className="flex flex-col sm:flex-row items-center gap-2 text-sm text-zinc-500">
                        <p>© {currentYear} DFDS.io Inc.</p>
                    ...
                    {compact && (
                         <div className="flex gap-4">
                            <SocialLink href="/twitter" icon={<Twitter className="w-4 h-4" />} label="Twitter" />
                            <SocialLink href="/github" icon={<Github className="w-4 h-4" />} label="GitHub" />
                        </div>
                    )}
